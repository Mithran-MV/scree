...
var configSchema = objectType({
  schedule: stringType(),
  surveyUrl: stringType().regex(/^https?:\/\/\S+$/),
  wallet: stringType().regex(/^0x[0-9a-fA-F]{40}$/),
  sources: stringType().optional(),
  policySecretId: stringType(),
  evm: objectType({
    chainSelectorName: stringType(),
    guardian: stringType().regex(/^0x[0-9a-fA-F]{40}$/),
    gasLimit: stringType().optional()
  }).optional()
});
var policySchema = objectType({
  minHealth: numberType().gt(1),
  floorPrice: numberType().gt(0),
  maxLift: numberType().gt(0)
});
var basketSchema = objectType({
...
var surveySchema = objectType({
  address: stringType(),
  spot: numberType().nullable(),
  baskets: arrayType(basketSchema),
  shape: stringType(),
  readAt: stringType()
});
var healthOf = (b, price) => {
  const debt = b.u * price + b.v;
  if (debt <= 0)
    return Number.POSITIVE_INFINITY;
  return (b.a * price + b.c) / debt;
};
var healthToday = (baskets, price) => baskets.reduce((low, b) => Math.min(low, healthOf(b, price)), Number.POSITIVE_INFINITY);
var crashPrice = (baskets, spot) => {
  let coast = null;
  for (const b of baskets) {
    if (b.a <= b.u)
      continue;
    const p = (b.v - b.c) / (b.a - b.u);
    if (p > 0 && p < spot && (coast === null || p > coast))
      coast = p;
  }
  return coast;
};
var VERDICT = { HOLD: 0, RAISE: 1, DROWNED: 2 };
var decide = (survey, policy) => {
  ...
  if (health < 1)
    return { verdict: "DROWNED", health, crash, lift: 0 };
  let lift = 0;
  if (health < policy.minHealth)
    lift = Math.max(lift, policy.minHealth - health);
  if (crash !== null && crash >= policy.floorPrice)
    lift = Math.max(lift, health * crash / policy.floorPrice - health);
  if (lift <= 0)
    return { verdict: "HOLD", health, crash, lift: 0 };
  return { verdict: "RAISE", health, crash, lift: Math.min(lift, policy.maxLift) };
};
var coarseBps = (x) => {
  if (!Number.isFinite(x))
    return 4294967295;
  return Math.min(4294967295, Math.max(0, Math.round(x * 100) * 100));
};
var onCronTrigger = (runtime) => {
  ...
  const encodedPayload = encodeAbiParameters(parseAbiParameters("address wallet, uint8 verdict, uint32 healthBps, uint32 liftBps, uint64 observedAt"), [config.wallet, VERDICT[decision.verdict], coarseBps(decision.health), coarseBps(decision.lift), observedAt]);
  ...
  const write = evm.writeReport(donRuntime, {
    receiver: config.evm.guardian,
    report,
  ...
  const txHash = bytesToHex3(write.txHash || new Uint8Array(32));
